...
(function () {
  ...
  function setStage(ix) {
    ...
    gsap
      ...
      .add(function () {
        stageWordEl.textContent = s.word;
        stageNoteEl.textContent = s.note;
      })
      .fromTo(
        [stageWordEl, stageNoteEl],
  ...
  function build3D() {
    ...
    renderer.setPixelRatio(
      Math.min(
        ...
        window.innerWidth < 720 ? 1.5 : 1.75,
      ),
    );
    ...
    var beanMat = new THREE.MeshStandardMaterial({
      ...
      roughness: 0.52,
      metalness: 0.12,
      emissive: new THREE.Color(0xff5f1e),
      emissiveIntensity: 0,
      transparent: true,
      opacity: 1,
    });
    ...
    var sparkTex = makeRadialTex([
      [0, "rgba(255,214,160,1)"],
      [0.3, "rgba(255,162,77,0.85)"],
      [1, "rgba(255,122,47,0)"],
    ]);
    ...
    var steamTex = makeRadialTex([
      [0, "rgba(248,244,240,0.65)"],
      [0.45, "rgba(238,230,222,0.25)"],
      [1, "rgba(238,230,222,0)"],
    ]);
    ...
    var haloTex = makeRadialTex([
      [0, "rgba(255,181,69,0.55)"],
      [0.4, "rgba(212,175,55,0.22)"],
      [1, "rgba(212,175,55,0)"],
    ]);
    ...
    function update(t, dt) {
      ...
      if (showBeans) {
        for (var i = 0; i < COUNT; i++) {
          ...
          var yy = lerp(
            b.y0 + Math.sin(t * b.bobW + b.ph) * b.bobA,
            CUP_Y - 0.2 - b.r0 * 0.14,
            e,
          );
      ...
      beanMat.emissiveIntensity =
        shot.ember *
        (0.5 + 0.12 * Math.sin(t * 7.3) + 0.08 * Math.sin(t * 11.7));
      beanMat.opacity = shot.beanFade;
      emberLight.intensity =
        shot.ember * (2.4 + 0.45 * Math.sin(t * 9.1)) + shot.ringGlow * 1.6;
      ...
      var pitch = clamp(
        shot.pitch + 0.015 * Math.sin(t * 1.3 + 1) + my * 0.03,
        -1.4,
        1.49,
      );
      var d = shot.dist;
  ...
  function exit() {
    ...
    if (scene3d) {
      tl.to(
        shot,
        { cremaCream: 1, cremaCalm: 0.9, duration: 1.05, ease: "sine.inOut" },
        0,
      );
    } else {
    ...
    tl.to(
      ...
      .add(
        announceDone,
        0.75,
      ) /* hero animation starts under the opening frame */
      .to(
        ".lc-bar-top",
        { yPercent: -101, duration: 1.05, ease: "power4.inOut" },
        0.7,
      )
      .to(
        ".lc-bar-bottom",
      ...
      .to(
        [canvas, ".lc-swirl2d", ".lc-vignette", ".lc-grain", ".lc-veil"],
        { opacity: 0, duration: 0.95, ease: "sine.inOut" },
        0.85,
      );
  }
  ...
  function tick() {
    ...
    if (scene3d) {
      playhead = lerp(
        playhead,
        (state.shown / 100) * CINE,
        clamp(dt * 3.0 * chaseBoost, 0, 1),
      );
